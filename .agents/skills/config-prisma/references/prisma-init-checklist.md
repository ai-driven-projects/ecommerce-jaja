...
```bash
node <SKILLS_DIR>/config-prisma/scripts/init-prisma-backend.js --dry-run
node <SKILLS_DIR>/config-prisma/scripts/init-prisma-backend.js --apply --install
...
```bash
node <SKILLS_DIR>/config-prisma/scripts/init-prisma-backend.js --apply --module auth --module stock --module billing
```
