...
14. Executar `config-shared-core` via `node <SKILLS_DIR>/config-shared-core/scripts/create-shared.mjs [--scope <namespace>]`:

- provisiona `<sharedModulePath>` (default: `packages/shared`) como submódulo Git
- valida contrato mínimo do pacote shared
- sincroniza `"<namespace>/shared": "*"` no frontend e backend
- executa `npm install` na raiz
- ignorado quando a raiz não for um repositório Git
