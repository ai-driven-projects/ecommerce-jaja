...
As configurações padrão são lidas de `skills.config.json` (na raiz do repositório consumidor ou em `.env/` do projeto).
...
12. Executar a skill `config-shared-core` (`node <SKILLS_DIR>/config-shared-core/scripts/create-shared.mjs`) para provisionar `packages/shared` como submódulo Git, validar o contrato mínimo e sincronizar a dependência `@<namespace>/shared` no frontend/backend (ignorado quando a raiz não for um repositório Git).
...
14. Formatar todos os arquivos do projeto com Prettier executando `npm run format` na raiz do monorepo.
...
```bash
node <SKILLS_DIR>/config-project/scripts/project-init.mjs
```
...
```bash
node <SKILLS_DIR>/config-project/scripts/project-init.mjs --skip-global-nest
```
...
```bash
node <SKILLS_DIR>/config-project/scripts/project-init.mjs --scope @namespace
```
...
- `../config-shared-core/scripts/create-shared.mjs`: provisionamento do pacote `packages/shared` (invocado automaticamente ao final do bootstrap).
