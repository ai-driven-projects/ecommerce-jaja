...
```bash
node <SKILLS_DIR>/backend-nest-config/scripts/apply.js
```

Use `--force` para sobrescrever mesmo arquivos sem alteração:

```bash
node <SKILLS_DIR>/backend-nest-config/scripts/apply.js --force
```

O script:

1. Usa o nome fixo do pacote shared (`@mentoria-360/shared`, independente do namespace do projeto)
...
O filtro `ApiExceptionFilter` importa diretamente do pacote shared (nome fixo `@mentoria-360/shared`):
