 ...
 *
 * Usage: node <SKILLS_DIR>/backend-nest-config/scripts/apply.js [--force]
 *
...
// Append an import after the existing import block (CRLF-safe).
...
function patchAppModule() {
  ...
  content = ensureImportLine(content, `import { SharedModule } from './shared/shared.module';`)
...
// ─── self-check (CRLF import insert) ──────────────────────────────────────────

if (process.argv.includes('--self-check')) {
  const line = "import { SharedModule } from './shared/shared.module';"
  const crlf = "import { A } from './a';\r\nimport { B } from './b';\r\n\r\nexport const x = 1;\r\n"
  const out = ensureImportLine(crlf, line)
  if (!out.includes(`from './b';\r\n${line}`)) {
    console.error(`${LOG} FAIL: import not inserted after CRLF block`)
    process.exit(1)
  }
  if (ensureImportLine(out, line) !== out) {
    console.error(`${LOG} FAIL: ensureImportLine not idempotent`)
    process.exit(1)
  }
  console.log(`${LOG} self-check ok`)
  process.exit(0)
}
