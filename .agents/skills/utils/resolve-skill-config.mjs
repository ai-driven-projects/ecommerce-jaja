...
// A configuração vive no projeto consumidor (raiz ou .env/), nunca dentro do
// diretório onde este repositório de skills foi montado.
const SKILL_CONFIG_DIR_CANDIDATES = ['.', '.env'];
const REPO_CONFIG_FILE = 'skills.config.json';
const LOCAL_CONFIG_FILE = 'skills.config.local.json';

// Nome fixo do pacote compartilhado. O shared é um submódulo Git externo com escopo
// próprio e não segue o namespace do projeto consumidor.
export const SHARED_PACKAGE_NAME = '@mentoria-360/shared';

const DEFAULT_CONFIG = {
  namespace: '',
  sharedModulePath: 'packages/shared',
  sharedSubmoduleUrl: 'https://github.com/mentoria-360/shared.git',
  frontendAppPath: 'apps/frontend',
...
function normalizeSubmoduleUrl(value, fallback) {
  if (typeof value !== 'string') return fallback;
  const trimmed = value.trim();
...
async function resolveConfigDir(rootDir) {
  // Usa o primeiro diretório que realmente contém um arquivo de configuração
  // (versionado ou local); a mera existência do diretório não basta.
  for (const relativeDir of SKILL_CONFIG_DIR_CANDIDATES) {
    ...
    const hasRepoConfig = await fileExists(path.join(candidate, REPO_CONFIG_FILE));
    const hasLocalConfig = await fileExists(path.join(candidate, LOCAL_CONFIG_FILE));
    if (hasRepoConfig || hasLocalConfig) {
...
export async function loadSkillConfig(rootDir) {
  ...
  const explicitSharedSubmoduleUrl = pickConfiguredString([localDefaults, repoDefaults], 'sharedSubmoduleUrl');
  ...
  return {
    defaults: {
      ...
      sharedModulePath,
      sharedSubmoduleUrl: normalizeSubmoduleUrl(
        explicitSharedSubmoduleUrl || mergedDefaults.sharedSubmoduleUrl,
        DEFAULT_CONFIG.sharedSubmoduleUrl,
      ),
      frontendAppPath,
      backendAppPath,
