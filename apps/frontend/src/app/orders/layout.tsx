'use client';

import { ArrowLeft, LayoutDashboard } from 'lucide-react';
import { PrivateAppShell } from '@/modules/auth/components/private-app-shell.component';
import { SidebarMenu, type SidebarMenuItem } from '@/shared/components/ui/sidebar-menu.component';

const ordersMenuItems: SidebarMenuItem[] = [
  {
    id: 'overview',
    label: 'Visão Geral Pedidos',
    href: '/orders',
    icon: LayoutDashboard,
    match: 'exact',
  },
];

function OrdersSidebarMenu() {
  return (
    <SidebarMenu
      mainItem={{
        id: 'back',
        label: 'Voltar',
        href: '/dashboard',
        icon: ArrowLeft,
      }}
      sections={[
        {
          id: 'orders',
          label: 'Pedidos',
          items: ordersMenuItems,
        },
      ]}
    />
  );
}

export default function OrdersModuleLayout({ children }: { children: React.ReactNode }) {
  return <PrivateAppShell sidebar={<OrdersSidebarMenu />}>{children}</PrivateAppShell>;
}
