...
O sistema SHALL criar um produto por `POST /products` a partir de `name`, `categoryId`, `priceCents` e, opcionalmente, `slug`, `sku`, `brandId`, `description`, `listPriceCents`, `unit`, `images` e `isActive`. Em sucesso MUST responder `201` com o produto completo: `id` (uuid v4 gerado pelo sistema), `name`, `slug`, `sku`, `brandId`, `categoryId`, `description`, `priceCents`, `listPriceCents`, `unit`, `images` (`thumbUrl`, `largeUrl`, `order`), `isActive`, `brandName` (nome da marca ou `null`), `categoryPath` (nomes da categoria e de seus ancestrais separados por ` / `), `createdAt` e `updatedAt`. Quando omitidos, `isActive` MUST ser `true`, `unit` MUST ser `"unidade"` e `images` MUST ser uma lista vazia; campos opcionais ausentes MUST ser `null`.
...
- **THEN** o sistema responde `201` com `slug: "caneta-esferografica-azul"`, `brandName: "BIC"`, `categoryPath: "Escrita & Corretivos / Canetas"`, `isActive: true`, `unit: "unidade"` e `images: []`
...
`GET /products` SHALL retornar `{ items, total, page, pageSize, totalPages }`, com os produtos não excluídos ordenados por nome. Cada item MUST conter `id`, `name`, `slug`, `sku`, `brandName`, `categoryPath`, `priceCents`, `listPriceCents`, `mainImageUrl` (miniatura da imagem principal ou `null`) e `isActive`. `page` MUST ter padrão `1` e `pageSize` padrão `20`; `pageSize` acima de `100` MUST ser limitado a `100`; `page` ou `pageSize` ausentes, não inteiros ou menores que `1` MUST assumir o padrão. `totalPages` MUST ser `ceil(total / pageSize)`. Uma página além da última MUST retornar `items` vazio com `total` e `totalPages` corretos.
...
`PUT /products/:id` SHALL aplicar ao produto todos os atributos enviados, com as mesmas validações e regras de unicidade, marca e categoria da criação, e responder `200` com o produto completo. A lista de imagens enviada MUST substituir integralmente a lista anterior (omitir `images` ou enviar lista vazia deixa o produto sem imagens). `id` e `createdAt` MUST NOT mudar e `updatedAt` MUST ser atualizado. Um `id` que nunca existiu MUST ser tratado como criação com esse `id` (responde `200` com o produto criado). Um `id` de produto excluído MUST responder `404` com `PRODUCT_NOT_FOUND`.
...
O sistema SHALL fornecer uma carga de dados que grava os produtos de `apps/backend/prisma/seed/data/products.json` (marca e categoria referenciadas por slug), executando depois das cargas de marcas e categorias; o backend MUST NOT ler arquivos do CLI. O CLI (`scrape:seed`) SHALL gerar esse arquivo a partir de todos os departamentos raspados da Kalunga, deduplicando os produtos pelo código da Kalunga (mantendo a primeira ocorrência, com os departamentos em ordem alfabética de arquivo), com 1.076 produtos. Na carga, um slug já usado por um produto que não está no arquivo MUST ser sufixado com `-<sku>`. Cada produto MUST ter: `sku` igual ao código da Kalunga; `slug` do arquivo normalizado e, se já usado por outro produto, sufixado com `-<sku>`; `name` do arquivo; a marca cujo slug é igual ao slug normalizado da marca do arquivo, ou nenhuma marca quando não houver correspondência; a categoria encontrada pelos nomes departamento → grupo → subgrupo (o grupo quando o subgrupo repete o nome do grupo), ou o departamento com um aviso no log quando não resolver; `description` com o texto da descrição limitado a 5000 caracteres; `priceCents` igual ao preço atual × 100 arredondado; `listPriceCents` igual ao preço "De:" × 100 arredondado somente quando maior que o preço atual, senão `null`; `unit` inferida do final do nome (`"CX N UN"` → `"caixa com N"`, `"PT N UN"` → `"pacote com N"`, demais casos → `"unidade"`); as imagens do arquivo na mesma ordem, limitadas às 10 primeiras; e `isActive` igual à disponibilidade. A carga MUST ser idempotente: executá-la de novo MUST atualizar os produtos pelo sku, substituindo as imagens, sem duplicar registros.
...
- **THEN** existem exatamente 1.076 produtos, todos com categoria e com ao menos uma imagem, e nenhum com mais de 10 imagens
