...
Define o comportamento da vitrine pública do Jaja na rota `/`: o catálogo real exibido a partir da API, como o visitante escolhe a loja e a categoria, busca, filtra e ordena produtos, o detalhe do produto e a passagem do carrinho para o checkout. O carrinho na loja é definido em `orders/storefront-cart`.
## Requirements
...
A vitrine e o detalhe do produto SHALL exibir o catálogo real lido da API pública da loja (`catalog/storefront-api`): produtos, categorias, marcas, preços e imagens visíveis. Os dados fictícios de produtos e categorias MUST NOT ser exibidos na vitrine nem no detalhe.

#### Scenario: Catálogo carregado
- **WHEN** a vitrine é exibida para a loja escolhida
...
O botão "Finalizar pedido" do carrinho SHALL fechar o painel do carrinho e navegar para `/checkout` preservando todos os parâmetros atuais da vitrine (loja, categoria, busca, filtros, ordenação e página). A navegação MUST NOT depender de sessão: quem não estiver autenticado é identificado na própria página de checkout, e os itens do carrinho do visitante seguem para a conta ao entrar (`orders/storefront-cart`). O botão MUST ficar indisponível quando o carrinho está vazio, quando tem itens indisponíveis ou enquanto uma mudança no carrinho não foi confirmada.

#### Scenario: Fechar pedido com loja e categoria
- **WHEN** um visitante com itens no carrinho em `/?loja=loja-paulista&categoria=escrita-corretivos` clica em "Finalizar pedido"
- **THEN** o painel do carrinho fecha e a página passa a `/checkout?loja=loja-paulista&categoria=escrita-corretivos`
...
O campo de busca do cabeçalho da loja SHALL buscar produtos. Enviar um termo, com Enter ou pelo botão da lupa, MUST navegar para `/` com `q=<termo>`, mantendo a `loja` e descartando categoria, filtros, ordenação e página. Enviar o campo vazio MUST remover `q`. O campo MUST exibir o termo presente na URL, inclusive após recarregar, e oferecer um botão para limpar o texto. A busca MUST funcionar também a partir do detalhe do produto.

#### Scenario: Buscar a partir da vitrine filtrada
- **WHEN** o visitante em `/?loja=loja-paulista&categoria=escolar&ordem=menor-preco` busca "caneta bic"
- **THEN** a URL passa a ser `/?loja=loja-paulista&q=caneta+bic`, e a listagem exibe "Resultados para “caneta bic”" com as canetas da BIC
...
- **THEN** navega para a listagem `/?q=papel` mantendo a loja
...
**Filtros ativos:** exibidos como pílulas removíveis, com a ação "Limpar filtros", que preserva `loja` e `q`.
...
- **WHEN** o visitante em `/?loja=loja-rio-branco&q=caneta&marca=bic&ofertas=1` clica em "Limpar filtros"
- **THEN** a URL passa a ser `/?loja=loja-rio-branco&q=caneta`
...
- **Imagem principal:** a grande, sem selo de tempo de entrega (não há cálculo de tempo nesta versão).
...
- **WHEN** o visitante acessa `/?loja=loja-paulista&categoria=escrita-corretivos`
